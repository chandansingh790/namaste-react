			...
			{item.data.map((items) => (
				<div key={items?.card?.info?.id} className="text-start m-2 p-2">
					<div className="justify-start mb-5 relative">
						<h1 className="font-semibold text-gray-700">{items?.card?.info?.name}</h1>
						<span>₹ {items?.card?.info?.price / 100}</span>
						<div className="w-24 h-24 border absolute right-1 bottom-0 top-2 bg-gray-100 rounded-lg">
							<img src={imageUrl + items?.card?.info?.imageId} className="relative"/>
                            <div className="w-20 h-8 border left-2 top-20 shadow-xl bg-white rounded-md text-center absolute text-green-500 font-bold"><button>ADD</button></div>
						</div>
					</div>
